...
typedef template_linear_rtModel RT_MODEL;

/*====================*
 * External functions *
 *====================*/
extern RT_MODEL * template_linear(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(template_linear_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: template_linear -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
